...
api.post("/assignments/add", (req, res) => {
  const { class_id, deadline_date, task_description } = req.body;

  if (!class_id || !deadline_date || !task_description) {
    return res.status(400).json({ error: "Missing required fields" });
  }

  const creation_date = new Date().toISOString().slice(0, 10); // 'YYYY-MM-DD'

  connection.query(
    "INSERT INTO assignments (class_id, creation_date, deadline_date, task_description) VALUES (?, ?, ?, ?)",
    [class_id, creation_date, deadline_date, task_description],
    (err, result) => {
      if (err) {
